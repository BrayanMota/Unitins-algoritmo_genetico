...
class Individuo():
    def __init__(self, dias, turnos, limite_dias, generation=0):
        self.dias = dias
        self.turnos = turnos
        self.limite_dias = limite_dias
    ...
    def crossover(self, other_individual):
        ...
        child1 = other_individual.chromosome[0:chromosome_split_mark] + \
            self.chromosome[chromosome_split_mark::]
        child2 = self.chromosome[0:chromosome_split_mark] + \
            other_individual.chromosome[chromosome_split_mark::]
...
class AG():
    ...
    def initialize_population(self, dias, turnos, limite_dias):
        for _ in range(self.population_size):
            self.population.append(Individuo(dias, turnos, limite_dias))
    ...
    def solve(self, taxa_mutacao, numero_geracoes, dias, turnos, limite_dias):

        self.initialize_population(dias, turnos, limite_dias)
        ...
        for _ in range(numero_geracoes):
            ...
            for __ in range(0, self.population_size, 2):
                ...
                children = self.population[father1].crossover(
                    self.population[father2])
                new_population.append(children[0].mutation(taxa_mutacao))
                new_population.append(children[1].mutation(taxa_mutacao))
